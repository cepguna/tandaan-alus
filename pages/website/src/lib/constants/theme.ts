...
export const grayColorsDefault = [
  {
    name: 'zinc',
    label: 'Shadcn',
    activeColor: {
...
// parse "240 5.9% 10%" → { h:240, s:5.9, l:10 }
function parseHsl(str: string): HSL {
  const [h, s, l] = str.split(' ').map(v => parseFloat(v));
  return { h, s, l };
}

// lighten by amt% (e.g. 5 → +5%)
function lighten(hsl: HSL, amt: number): HSL {
  return { ...hsl, l: Math.min(hsl.l + amt, 100) };
}

// stringify back to "240 5.9% 10%"
function toString({ h, s, l }: HSL): string {
  return `${h} ${s}% ${l}%`;
}

// amount you want to brighten the sidebar bg by:
const SIDEBAR_LIGHTEN = 5;

export const grayColors = grayColorsDefault.map(theme => {
  const { light, dark } = theme.cssVars;

  // compute new sidebar‑bg for light + dark
  const sbDarkBg = toString(lighten(parseHsl(dark.background), SIDEBAR_LIGHTEN));
  ...
  return {
    ...
    cssVars: {
      light: {
        ...light,
        // sidebar bg is main bg + lighten
        'sidebar-background': '0 0% 98%',
        // keep sidebar text same as theme fg
        'sidebar-foreground': '240 5.3% 26.1%',
        // you can still hard‑code your primary/accent/border/ring or derive them too
      ...
      dark: {
        ...dark,
        'sidebar-background': sbDarkBg,
        'sidebar-foreground': dark.foreground,
        'sidebar-primary': dark.accent,
        'sidebar-primary-foreground': dark['accent-foreground'],
        'sidebar-accent': dark.accent,
        'sidebar-accent-foreground': dark['accent-foreground'],
        'sidebar-border': dark.border,
        'sidebar-ring': dark.accent,
      },
    },
  };
});
